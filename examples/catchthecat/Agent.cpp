...
int Agent::getHeuristic(Point2D point, int sideSize) {
  ...
  if(point.x + point.y > 0) {
    if(point.x - point.y > 0) {
      return abs((sideSize / 2) - point.x);
    }
    else {
      return abs((sideSize / 2) - point.y);
    }
  }
  else {
    if (point.x - point.y > 0) {
...
std::vector<Point2D> Agent::generatePath(World* w) {
  ...
  while (!queue.empty()) {
    ...
    std::vector<Point2D> neighbors = getVisitableNeighbors(w, current.point);
    for (auto neighbor : neighbors) {
      if (!visited[neighbor]) {
        // Push to the queue
        cameFrom.emplace(neighbor, current.point);
        queue.push({neighbor, current.accDist+1, getHeuristic(neighbor, w->getWorldSideSize())});
      }
    }
  ...
  if (borderExit != Point2D::INFINITE) {
    Point2D pathPoint = borderExit;
    while (pathPoint != catPos) {
...
std::vector<Point2D> Agent::getVisitableNeighbors(World* w, Point2D p) {
  ...
  if (w->isValidPosition(nextPoint) && !w->getContent(nextPoint)) {
